...
* ✂️ **Pipeline di Ritaglio Verticale 9:16:** Taglio intelligente delle scene salienti con centraggio automatico del soggetto.
* 🎙️ **Sottotitoli Dinamici Parlati (Stile Hormozi):** Trascrizione audio parola per parola con evidenziazione colorata ad alto impatto.
* 🛡️ **Auditor Anti-Rifiuto & Cascata 1+6:** Monitoraggio continuo dello stato di approvazione su Klippify e adattamento istantaneo di Hook Card e Outro CTA (tasso di approvazione al **100%**).
...
│   └── generate_report.py          # Generatore del report HTML/Markdown e widget reattivi
│
├── ⚙️ MOTORE AUTOPILOTA & LOGICA
│   ├── autopilot_engine.py         # Loop di pubblicazione ciclica e gestione orari
│   ├── campaign_classifier.py      # Analisi e ranking di fattibilità delle campagne
│   └── process_manager.py          # Gestore dei processi in background
│
├── 🎬 CATENA DI MONTAGGIO VIDEO
│   ├── source_downloader.py        # Downloader video con calcolo score virale (yt-dlp)
│   ├── clipping_queue_manager.py   # Coda di elaborazione per la materia prima
│   ├── clipping_pipeline.py        # Segmentazione e montaggio in formato verticale 9:16
│   └── video_captioner.py          # Generazione sottotitoli sincronizzati e Hook/Outro card
│
├── 🛡️ CONTROLLO QUALITÀ & AUDIT
│   ├── rejection_auditor.py        # Audit automatico delle sottomissioni Klippify ogni 4h
│   └── watcher_instructions.md     # Protocollo operativo di auto-correzione
│
├── 📱 SOCIAL & CARICAMENTO
│   ├── tiktok_uploader.py          # Modulo di caricamento video su TikTok
│   └── tiktok_manager.py           # Gestione token e sessioni creator
│
├── 📊 DATABASE & STATI (JSON)
...
│   └── klippify_submissions.json   # Cache locale delle sottomissioni e approvazioni
│
└── 🚀 LAUNCHER RAPIDI
